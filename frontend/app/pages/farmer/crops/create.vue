<template>
  <div class="p-6 min-h-[calc(100vh-5rem)] flex flex-col items-center justify-center space-y-8 text-[#112830]">
    ...
    <div class="w-full max-w-xl space-y-2 text-center mb-4">
      ...
      <h1 class="text-4xl font-black tracking-tighter">Nouvelle Culture</h1>
      <p class="text-gray-500 font-medium max-w-md mx-auto">Enregistrez une nouvelle culture pour commencer le suivi de vos parcelles.</p>
    </div>
    ...
    <div class="w-full max-w-xl bg-white p-10 rounded-[3rem] border border-gray-100 shadow-[0_20px_50px_rgba(0,0,0,0.04)] relative overflow-hidden">
      ...
      <form @submit.prevent="submitCrop" class="relative z-10 space-y-8">
        <!-- Crop Name -->
        <div class="space-y-3">
          <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-4">{{ t("cropname") }} *</label>
        ...
        <!-- Variety Selection -->
        <div class="space-y-3">
          <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-4">{{ t("variety") }} *</label>
          <div class="relative group">
    ...
    <!-- Notification system (Global Transition) -->
...
const submitCrop = async () => {
  ...
  try {
    // ✅ CORRIGÉ : on utilise directement apiFetch, pas de `res` fantôme
