          ...
          >
            <span
              :class="[
                ...
                day.events.length
                  ? taskPriorities.value[day.events[0].priority] === 'High'
                    ? 'bg-[#e63946]/10 text-[#e63946] border border-[#e63946]/50e'
                    : taskPriorities.value[day.events[0].priority] === 'Medium'
          ...
          <div class="flex flex-col sm:flex-row gap-4">
            ...
            <div class="flex-1 flex flex-col">
              ...
              <select
                ...
                class="w-full p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#10b481] focus:border-transparent transition"
              >
                <option v-for="p in taskPriorities" :key="p.id" :value="p.id">
...
const prioritiesJson = computed(() =>
  JSON.stringify(taskPriorities.value, null, 2)
);
...
onMounted(async () => {
  ...
  try {
    taskPriorities.value = await apiFetch('/api/task-priority/');
    taskStatuses.value = await apiFetch('/api/task-status/');

    taskStatuses.value.forEach((s) => {
...
async function createTask() {
  ...
  try {
    ...
    tasks.value.push(created as any);
    selectedEvents.value.push(created as any);
...
const tasksByPriority = computed(() => {
  ...
  taskPriorities.value.forEach((p) => {
...
const tasksByStatus = computed(() => {
  ...
  taskStatuses.value.forEach((s) => (counts[s.id] = 0));
