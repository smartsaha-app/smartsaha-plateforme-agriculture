<template>
  <div class="min-h-screen bg-[#f8fafc] p-6 md:p-8 space-y-6">
    ...
    <!-- ===== HEADER ===== -->
    <PageHeader :title="t('organization.requestsTitle')">
      <template #subtitle>
        <i class="bx bx-envelope"></i>
        {{ t('organization.requestsDesc') }}
        <span class="ml-2 px-2.5 py-0.5 bg-amber-100 text-amber-600 rounded-lg text-[9px] font-black uppercase tracking-widest border border-amber-200">
          {{ activeTab === 'requests' ? pendingRequests.length : pendingInvitations.length }}
          {{ activeTab === 'requests' ? 'demande(s)' : 'invitation(s)' }}
        </span>
      </template>
      <template #breadcrumb>
        ...
        <span class="text-[#10b481]">{{ t('organization.requestsTitle') }}</span>
      </template>
    </PageHeader>
    ...
    <div v-if="isLoading" class="flex flex-col items-center justify-center py-20 space-y-4">
      ...
      <p class="text-gray-400 font-bold animate-pulse">{{ t('dashboard.loading') }}</p>
    </div>

    <!-- ===== ONGLET : DEMANDES REÇUES ===== -->
    <div v-else-if="activeTab === 'requests'">
      <div v-if="pendingRequests.length === 0" class="bg-white p-20 rounded-2xl text-center border border-dashed border-gray-200">
        <div class="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-6 text-gray-200 text-4xl">
          <i class="bx bx-user-voice"></i>
        </div>
        <h3 class="text-xl font-bold text-gray-400">{{ t('organization.noRequests') }}</h3>
        <p class="text-gray-300 mt-2">{{ t('organization.noRequestsDesc') }}</p>
      </div>

      <div v-else class="grid grid-cols-1 gap-4">
        <div v-for="req in pendingRequests" :key="req.uuid"
             class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl transition-all flex flex-col md:flex-row items-center justify-between gap-6 group">
          <div class="flex items-center gap-6">
            ...
            <div>
              ...
              <p class="text-sm text-gray-400 font-medium">{{ t('organization.wantsToJoin') }} <span class="text-[#10b481] font-bold">{{ req.group.name }}</span></p>
          ...
          <div class="flex items-center gap-3">
            <button @click="openApproveModal(req)" class="px-6 py-3 bg-[#10b481] text-white rounded-2xl font-bold hover:bg-[#112830] transition-all flex items-center gap-2 shadow-lg shadow-[#10b481]/10">
              <i class="bx bx-check"></i> {{ t('organization.accept') }}
            </button>
            <button @click="handleReject(req)" class="px-6 py-3 bg-white border border-rose-100 text-rose-500 rounded-2xl font-bold hover:bg-rose-50 transition-all flex items-center gap-2">
              <i class="bx bx-x"></i> {{ t('organization.reject') }}
            </button>
          </div>
        </div>
      </div>
    ...
    <div v-else-if="activeTab === 'invitations'">
      <div v-if="pendingInvitations.length === 0" class="bg-white p-20 rounded-2xl text-center border border-dashed border-gray-200">
        <div class="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-6 text-gray-200 text-4xl">
          <i class="bx bx-send"></i>
        </div>
        <h3 class="text-xl font-bold text-gray-400">{{ t('organization.noInvitations') }}</h3>
        <p class="text-gray-300 mt-2">{{ t('organization.noInvitationsDesc') }}</p>
      </div>
        ...
        <div v-for="inv in pendingInvitations" :key="inv.uuid"
          ...
          <div class="flex items-center gap-6">
            ...
            <div>
              ...
              <p class="text-sm text-gray-400 font-medium">{{ t('organization.invitedToJoin') }} <span class="text-blue-500 font-bold">{{ inv.group.name }}</span></p>
          ...
          <div class="flex items-center gap-3">
            <span class="px-4 py-2 bg-amber-50 text-amber-500 rounded-2xl text-xs font-black uppercase tracking-widest border border-amber-100">
              {{ t('organization.pendingResponse') }}
            </span>
            <button @click="openCancelModal(inv)" class="px-6 py-3 bg-white border border-rose-100 text-rose-500 rounded-2xl font-bold hover:bg-rose-50 transition-all flex items-center gap-2">
              <i class="bx bx-x-circle"></i> {{ t('organization.cancelInviteBtn') }}
            </button>
          </div>
        </div>
      </div>
    ...
    <div v-if="selectedRequest" class="fixed inset-0 z-[100] flex items-center justify-center p-6 backdrop-blur-md bg-black/20">
      <div class="bg-white w-full max-w-lg rounded-2xl p-10 shadow-2xl space-y-6 relative border border-white/20">
        ...
        <div class="text-center space-y-2">
          ...
          <h2 class="text-2xl font-black text-[#112830]">{{ t('organization.validateMembership') }}</h2>
        ...
        <div class="space-y-4 pt-4">
          <label class="text-xs font-black text-gray-400 uppercase tracking-widest">{{ t('organization.roleInGroup') }}</label>
        ...
        <div class="flex gap-3 pt-6">
          <button @click="selectedRequest = null" class="flex-1 py-4 text-gray-400 font-bold hover:text-gray-600">{{ t('dashboard.cancel') }}</button>
    ...
    <div v-if="selectedInvitation" class="fixed inset-0 z-[100] flex items-center justify-center p-6 backdrop-blur-md bg-black/20">
      <div class="bg-white w-full max-w-md rounded-2xl p-10 shadow-2xl space-y-6 relative border border-white/20">
        ...
        <div class="text-center space-y-2">
          <div class="w-20 h-20 rounded-xl bg-rose-50 text-rose-400 flex items-center justify-center mx-auto mb-4">
            <i class="bx bx-x-circle text-4xl"></i>
          </div>
          <h2 class="text-2xl font-black text-[#112830]">{{ t('organization.cancelInviteModalTitle') }}</h2>
        ...
        <div class="flex gap-3 pt-4">
          ...
          <button @click="confirmCancel" :disabled="isCancelling" class="flex-2 py-4 px-10 bg-rose-500 text-white rounded-2xl font-bold hover:bg-rose-600 shadow-lg shadow-rose-500/20 disabled:opacity-50 transition-all flex items-center gap-2 justify-center">
            ...
            {{ t('organization.cancelInviteModalTitle') }}
          </button>
        </div>
      </div>
    </div>
    ...
    <transition name="slide-up">
      <div v-if="toast.visible"
        class="fixed bottom-6 left-1/2 -translate-x-1/2 z-[200] px-5 py-3 rounded-2xl shadow-xl flex items-center gap-3 text-sm font-bold"
...
async function confirmApprove() {
  ...
  try {
    ...
    showToast("Adhésion approuvée avec succès !");
  } catch (err) {
...
async function handleReject(req: any) {
  try {
    ...
    showToast("Une erreur est survenue lors du refus.", 'error');
  }
}
